...
export const getAllProducts = async (req, res) => {
  try {
    const { categoryId, search, page, limit, inStock, inWishlist, sortBy, order, userId } = req.query;
    const result = await productService.getAllProducts({ categoryId, search, page, limit, inStock, inWishlist, sortBy, order, userId }, req);
    return ApiResponse.ok(res, result, "Products fetched successfully");
  } catch (err) {
    return ApiResponse.error(res, err.message, err.status || 500);
  }
